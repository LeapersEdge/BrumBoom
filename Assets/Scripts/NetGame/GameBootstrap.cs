...
namespace NetGame
{
    ...
    public class GameBootstrap : MonoBehaviour, INetworkRunnerCallbacks
    {
        ...
        [SerializeField] private bool includeInactiveSpawnPoints;
        ...
        public void RefreshSpawnPointsFromRoot()
        {
            AutoAssignSpawnPointsRoot();
            if (spawnPointsRoot != null)
                RebuildSpawnPointsFromRoot();
        ...
        private void AutoAssignSpawnPointsRoot()
        {
            if (spawnPointsRoot != null)
                return;
            ...
            if (go != null)
                spawnPointsRoot = go.transform;
        }

        private void RebuildSpawnPointsFromRoot()
        {
            spawnPoints.Clear();
            if (includeInactiveSpawnPoints)
            {
                spawnPoints.AddRange(spawnPointsRoot.GetComponentsInChildren<Transform>(true));
            }
            else
            {
                spawnPoints.AddRange(spawnPointsRoot.GetComponentsInChildren<Transform>());
            }

            // remove the root itself; keep only children
            spawnPoints.Remove(spawnPointsRoot);
        }
